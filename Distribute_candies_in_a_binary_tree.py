...
# Given a binary tree with N nodes, in which each node value represents number of candies present at that node. In one move, one may choose two adjacent nodes and move only one candy 
# from one node to another (the move may be from parent to child, or from child to parent.) 
# The task is to find the number of moves required such that every node has exactly one candy.
# Note that the testcases are framed such that it is always possible to achieve a configuration 
...
# distributeCandy() which takes the root of the tree as input and returns the number of 
# moves required such that every node has exactly one candy.
